...
public class RemoveTag extends CommandBase {
    @Override
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if(!PluginWrapper.containsEnchantment(meta, tag)) {
            sender.sendMessage("This item does not have that tag!");
            return true;
        }
        ...
        if(meta.hasLore()) {
            ...
            builder.removeLine(tag.getAbilityDescription()[0]);
            meta.setLore(builder.build());
        }
        meta.removeEnchant(tag);
        handItem.setItemMeta(meta);
        sender.sendMessage("Removed tag!");
        return true;
    }
